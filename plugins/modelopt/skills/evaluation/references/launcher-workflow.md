...
- [Step 2 — Build base config (when not using shortcut)](#step-2--build-base-config-when-not-using-shortcut)
- [Step 3 — Configure deployment](#step-3--configure-deployment)
- [Step 4 — Fill remaining ??? values](#step-4--fill-remaining--values)
- [Step 5 — Confirm tasks (iterative)](#step-5--confirm-tasks-iterative)
- [Step 6 — Multi-node](#step-6--multi-node)
- [Step 7 — Interceptors](#step-7--interceptors)
- [Step 7.5 — Container registry auth (SLURM private images only)](#step-75--container-registry-auth-slurm-private-images-only)
- [Step 8 — Run evaluation (gated dry-run → canary → full)](#step-8--run-evaluation-gated-dry-run--canary--full)
