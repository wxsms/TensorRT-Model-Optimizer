...
- Fix NemotronH dense MLP quantization with the ``nvfp4_mlp_only`` and ``nvfp4_omlp_only`` recipe families. NemotronH registers these projections as ``mixer.up_proj`` and ``mixer.down_proj``, which were missed by the previous ``*mlp*`` selector and produced checkpoints with a null ``quant_algo`` (NVBug 6571812).
